...
function updateChart() {
  ...
  Object.keys(dataTerbaru).forEach((key) => {
    const [year, month, day] = key.split("-").map(Number);
    if (year === currentYear) {
      if (dataTerbaru[key] === "succeed") monthlyData[month].succeed++;
      else if (dataTerbaru[key] === "failed") monthlyData[month].failed++;
    }
  });
  ...
  myBarChart = new Chart(ctxBar, {
    ...
    data: {
      ...
      datasets: [
        {
          label: "Berhasil",
          data: monthlyData.map((d) => d.succeed),
          backgroundColor: "#01e235",
        },
        {
          label: "Gagal",
          data: monthlyData.map((d) => d.failed),
...
document.getElementById("chart-filter").addEventListener("change", updateChart);
